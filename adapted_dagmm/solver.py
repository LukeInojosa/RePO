...
class Solver(object):
    ...
    def build_model(self):
        # Define model
        input_data, _ = next(iter(self.data_loader))
        self.dagmm = DaGMM(self.gmm_k, input_size=input_data.shape[1])
    ...
    def train(self):
        ...
        for e in range(start, self.num_epochs):
            for i, (input_data, labels) in enumerate(tqdm(self.data_loader)):
                ...
                # Print out log info
                if self.log_step is not None and (i+1) % self.log_step == 0:
    ...
    def test(self, custom_input = None):
        print("======================TEST MODE======================")
        self.dagmm.eval()
        ...
        if custom_input is None:
            ...
            test_energy = []
            test_z = []
            for input_data in custom_input:
